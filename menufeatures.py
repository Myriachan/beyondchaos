from utils import (Substitution, pointer16_to_bytes, pointer24_to_bytes,
    get_asm_patch, apply_asm_patch, use_new_asm_storage)
...
def add_common_menu_stuff(fout):
    ...
    apply_asm_patch(fout, "menu_common")

def improve_item_display(fout):
    add_common_menu_stuff(fout)

    # TEMPORARY HACK
    use_new_asm = use_new_asm_storage[0]
...
def show_original_names(fout):
    """ Show the original name of each character on some screens. """
    add_common_menu_stuff(fout)

    apply_asm_patch(fout, "show_original_names")
